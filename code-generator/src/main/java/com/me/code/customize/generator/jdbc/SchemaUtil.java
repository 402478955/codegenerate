...
public class SchemaUtil {
     ...
     * COLUMN_KEY 列的键属性
     */
    private static String COLUMN_TEMPLATE = "select t.COLUMN_NAME, upper(t.DATA_TYPE), t.COLUMN_COMMENT, upper(t.COLUMN_KEY),column_default FROM information_schema.COLUMNS t where t.TABLE_SCHEMA = '$tableSchema' and t.TABLE_NAME = '$tableName' ORDER BY t.ORDINAL_POSITION";
    ...
    public static List<TableInfo> getTableInfos(String tableSchema, List<String> tableNames) throws SQLException {
        ...
        for (TableInfo tableInfo : tableInfos) {
            ...
            ResultSet resultSet = DbUtil.executeQuery(
                    COLUMN_TEMPLATE.replace("$tableSchema", tableSchema).replace("$tableName", tableInfo.getTableName().toLowerCase()));
            if (resultSet == null) {
                continue;
            }
